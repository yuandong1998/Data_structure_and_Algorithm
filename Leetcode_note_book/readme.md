...
* [★★填充每个节点的下一个右侧节点指针](./notebook/填充每个节点的下一个右侧节点指针.md) （用队列进行层序遍历，在遍历时记录上一个访问的节点）

* [★★★树中距离之和](./notebook/树中距离之和.md) （动态规划）

* [★★实现Trie(前缀树)](./notebook/实现Trie(前缀树).md)  （树的每一个节点包括指向子节点的指针数组`vector<Trie*> children`和该节点是否是字符串的结束`isEnd`）

* [★★单词频率](./notebook/单词频率.md) （Trie树来实现）
...
* [乘积最大子数组](./notebook/乘积最大子数组.md) 2020/9/11
...
* [★数组中出现次数超过一半的数字](./notebook/数组中出现次数超过一半的数字.md) （二分法和Boyer-Moore 投票算法）
...
* [★★ 二叉搜索树的后序遍历序列](./notebook/二叉搜索树的后序遍历序列.md) （递归；单调栈，对后序遍历数组反向遍历，用单调栈维护一个最小上界）
...
哈希
* [字母异位词分组](./notebook/字母异位词分组.md)

十、其他

* [★★数组中数字出现的次数](./notebook/数组中数字出现的次数.md)（分组异或，先对所有数字进行一次异或，在异或结果中找到任意为 1 的位对所有数进行分组，再分组异或）
* [★★数组中数字出现的次数Ⅱ](./notebook/数组中数字出现的次数Ⅱ.md)（数组中所有数字的二进制对每一位求和，能被3整除的位，目标数在该位为0，否则为1）
